...
<body>
	<nav class="breadcrumb">
		...
		症状管理 <span class="c-gray en">&gt;</span> 症状列表 <a
	...
	<div class="page-container">
		<div class="text-c">
			...
			<input type="text" name="" id="" placeholder=" 症状名称"
				style="width: 250px" class="input-text">
			<button name="" id="" class="btn btn-success" type="submit">
				<i class="Hui-iconfont">&#xe665;</i> 症状
			</button>
		</div>
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"><a href="javascript:;" onclick="datadel()"
				class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i>
					批量删除</a> <a class="btn btn-primary radius" data-title="添加症状"
				...
				<table
					...
					<thead>
						<tr class="text-c">
							...
							<th width="40">序号</th>
							<th width="250">ID</th>
							<th width="200">症状名称</th>
							<th>症状简介</th>
							<th width="120">操作</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${symptomWithBLOBsList }" var="symptomWithBLOBs"
							varStatus="status">
							<tr class="text-c">
								<td><input type="checkbox" value="" name=""></td>
								<td>${status.index + 1}</td>
								<td>${symptomWithBLOBs.symptomId }</td>
								<td><a style="cursor: pointer" class="text-primary"
									href="${pageContext.request.contextPath }/symptom/showSymptom?symptomId=${symptomWithBLOBs.symptomId }">${symptomWithBLOBs.symptomName }</a></td>
								<td><c:choose>
										<c:when test="${fn:length(symptomWithBLOBs.symptomIntro)>20}">${fn:substring(symptomWithBLOBs.symptomIntro,0 ,20 )}......</c:when>
										<c:otherwise>${symptomWithBLOBs.symptomIntro}</c:otherwise>
									</c:choose></td>
								...
								<td class="f-14 td-manage"><a style="text-decoration: none"
									...
									onClick="article_edit('症状编辑','article-add.html','10001')"
									href="${pageContext.request.contextPath }/symptom/detailedSymptom?symptomId=${symptomWithBLOBs.symptomId }"
