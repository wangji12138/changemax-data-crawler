...
<body>
	<nav class="breadcrumb">
		...
		疾病管理 <span class="c-gray en">&gt;</span> 疾病列表 <a
	...
	<div class="page-container">
		<div class="text-c">
			...
			<input type="text" name="" id="" placeholder=" 疾病名称"
				style="width: 250px" class="input-text">
			<button name="" id="" class="btn btn-success" type="submit">
				<i class="Hui-iconfont">&#xe665;</i> 疾病
			</button>
		</div>
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"><a href="javascript:;" onclick="datadel()"
				class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i>
					批量删除</a> <a class="btn btn-primary radius" data-title="添加疾病"
				data-href="article-add.html" onclick="Hui_admin_tab(this)"
				href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加疾病</a></span> <span
		...
		<div class="mt-20">
			<form id="diseaseForm" name="diseaseForm"
				action="${pageContext.request.contextPath }/disease/listDisease"
				method=post>
				<table
					...
					<tbody>
						<c:forEach items="${diseaseWithBLOBsList }" var="diseaseWithBLOBs"
							varStatus="status">
							<tr class="text-c">
								<td><input type="checkbox" value="" name=""></td>
								<td>${status.index + 1}</td>
								<td>${diseaseWithBLOBs.diseaseId }</td>
								<td><a style="cursor: pointer" class="text-primary"
									href="${pageContext.request.contextPath }/disease/showDisease?diseaseId=${diseaseWithBLOBs.diseaseId }">${diseaseWithBLOBs.diseaseName }</a></td>
								<td><c:choose>
										<c:when test="${fn:length(diseaseWithBLOBs.diseaseIntro)>20}">${fn:substring(diseaseWithBLOBs.diseaseIntro,0 ,20 )}......</c:when>
										<c:otherwise>${diseaseWithBLOBs.diseaseIntro}</c:otherwise>
									</c:choose></td>
								...
								<td class="f-14 td-manage"><a style="text-decoration: none"
									...
									onClick="article_edit('疾病编辑','article-add.html','10001')"
									href="${pageContext.request.contextPath }/disease/detailedDisease?diseaseId=${diseaseWithBLOBs.diseaseId }"
