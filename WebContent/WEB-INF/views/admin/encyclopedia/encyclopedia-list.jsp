...
<body>
	<nav class="breadcrumb">
		...
		百科管理 <span class="c-gray en">&gt;</span> 百科列表 <a
	...
	<div class="page-container">
		<div class="text-c">
			...
			<input type="text" name="" id="" placeholder=" 百科名称"
				style="width: 250px" class="input-text">
			<button name="" id="" class="btn btn-success" type="submit">
				<i class="Hui-iconfont">&#xe665;</i> 百科
			</button>
		</div>
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"><a href="javascript:;" onclick="datadel()"
				class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i>
					批量删除</a> <a class="btn btn-primary radius" data-title="添加百科"
				data-href="article-add.html" onclick="Hui_admin_tab(this)"
				href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加百科</a></span> <span
		...
		<div class="mt-20">
			...
			<form id="encyclopediaForm" name="encyclopediaForm"
				action="${pageContext.request.contextPath }/encyclopedia/listEncyclopedia"
				method=post>
				<table
					...
					<thead>
						<tr class="text-c">
							...
							<th width="250">ID</th>
							<th>百科名称</th>
							<th width="40">第三方链接</th>
							<th width="120">操作</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${encyclopediaList }" var="encyclopedia"
							varStatus="status">
							<tr class="text-c">
								<td><input type="checkbox" value="" name=""></td>
								<td>${status.index + 1}</td>
								<td>${encyclopedia.id }</td>
								<td><a style="cursor: pointer" class="text-primary"
									href="${pageContext.request.contextPath }/encyclopedia/showEncyclopedia?name=${encyclopedia.name }&url=${encyclopedia.url }">${encyclopedia.name }</a></td>
								<td><a href="${encyclopedia.url }">第三方链接地址</a></td>
								<td class="f-14 td-manage"><a style="text-decoration: none"
									...
									onClick="article_edit('百科编辑','article-add.html','10001')"
									href="${pageContext.request.contextPath }/encyclopedia/detailedEncyclopedia?id=${encyclopedia.id }"
	...
	<script type="text/javascript">
		function to_page(pageIndex) {
			...
			document.encyclopediaForm.submit();
		}
	</script>
</body>
</html>